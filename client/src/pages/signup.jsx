...
const signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setIsLoading(true)
      const response = await axios.post('http://localhost:3001/send-mail', {register : true, name, email, password })
      ...
      {isLoading ? <Spinner/> : (!isCodeSent ? 
        <form onSubmit={handleSubmit} className='flex flex-col gap-5 '>
          <div className='flex gap-5'>
            ...
            <input type="text" placeholder='Enter your name' value={name} onChange={(e) => setName(e.target.value)} className='border p-2 rounded-md' />
          </div>
          <div className='flex gap-5'>
